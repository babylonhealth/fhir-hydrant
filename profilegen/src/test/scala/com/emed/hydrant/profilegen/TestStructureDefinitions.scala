...
object TestStructureDefinitions {
  ...
  lazy val structureDefinitions = new StructureDefinitions(
    {
      for {
        fileName <- fileNames
        file = slurpRsc(s"profiles/$fileName")
        structureDef <- decode[StructureDefinition](file).toOption
      } yield structureDef
    },
    Seq(decode[StructureDefinition](slurpRsc("profiles/bbl/bblObservation.json")).toTry.get))
